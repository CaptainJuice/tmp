...
Same protocol as Figure 5: $`N = 300`$, $`\psi = 64`$, $`t = 200`$ trees, single linkage. The geodesic distance is computed via $`k`$-NN graph
...
GF performs poorly throughout ( $`\leq 0.47`$ ), confirming its
...
distances to $`S_i`$. The plotted value is the average over all $`N`$ points.
